...
#pragma warning restore CA1307 // Specify StringComparison
            ...
            if (systemCodes.Any())
            {
                var valueSetGuids = this.sharedContext.ValueSetCodes.Where(code => systemCodes.Contains(code.CodeSystemGuid))
                    .Select(code => code.ValueSetGUID)
                    .Distinct();

                dtos = dtos.Join(valueSetGuids, dto => dto.ValueSetGUID, key => key, (dto, key) => dto);
            }
